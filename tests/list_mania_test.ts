...
import { ListMania } from "./list_mania.capnp.ts";
import { assertDoesNotThrow } from "../test_utils.ts";

Deno.test("loop de loop", () => {
  assertDoesNotThrow(() => {
    ...
    assert(s.isWordZero(0x0a0));
    assert(s.isWordZero(0x118));
  });
});

Deno.test("1 of each list", () => {
  assertDoesNotThrow(() => {
    const m = new capnp.Message();
    const listMania = m.initRoot(ListMania);
    ...
    const s = m.getSegment(0);
    for (let i = 8; i < s.byteLength; i += 8) {
      assert(s.isWordZero(i));
    }
  });
});
