import { moduleUrl } from "../mod.ts";
import { dirname, fromFileUrl, relative } from "./path_utils.ts";
...
function getCapnpTsImportSpecifier(tsImporterPath: string): string {
  if (!moduleUrl.startsWith("file://")) {
    return moduleUrl;
  }
  const modulePath = fromFileUrl(moduleUrl);
  const relativePath = relative(dirname(tsImporterPath), modulePath);
  return relativePath;
}

export function generateCapnpImport(ctx: CodeGeneratorFileContext): void {
  const importPath = getCapnpTsImportSpecifier(ctx.tsPath);
...
export function generateNestedImports(ctx: CodeGeneratorFileContext): void {
  ctx.imports.forEach((i) => {
    ...
    const imports = getImportNodes(ctx, lookupNode(ctx, i)).map(
      getFullClassName,
    ).join(", ");

    if (imports.length < 1) return;

    // TODO: Need to investigate global resolution as documented in
    // https://capnproto.org/cxx.html#setting-a-namespace.
    let importPath: string;
    if (name.substring(0, 7) === "/capnp/") {
      const moduleSpecifier = getCapnpTsImportSpecifier(ctx.tsPath);
      if (!moduleSpecifier.endsWith("/mod.ts")) {
        throw new Error(
          `Expected module specifier to end with "/mod.ts", but got "${moduleSpecifier}"`,
        );
      }
      importPath = `${
        moduleSpecifier.slice(0, moduleSpecifier.length - 7)
      }/lib/std/${name.substring(7)}.ts`;
    } else {
      importPath = name[0] === "." ? `${name}.ts` : `./${name}.ts`;
    }
...
export function generateStructFieldMethods(
  ...
  const hadExplicitDefault = field.isSlot() &&
    field.getSlot().getHadExplicitDefault();
  ...
  switch (whichType) {
    ...
    case s.Type.LIST: {
      const whichElementType = field.getSlot().getType().getList()
        .getElementType().which();
      let listClass = ConcreteListType[whichElementType];

      if (
        whichElementType === s.Type.LIST || whichElementType === s.Type.STRUCT
      ) {
  ...
  if (adopt) {
    const parameters = [
      f.createParameterDeclaration(__, __, VALUE, __, orphanType, __),
    ];
    const expressions = [
...
export function generateStructNode(
  ...
  const concreteLists = fields.filter(needsConcreteListClass).sort(
    compareCodeOrder,
  );
  const consts = ctx.nodes.filter((n) =>
    n.getScopeId() === nodeId && n.isConst()
  );
  ...
  const defaultValues = fields.reduce(
    (acc, f) =>
      f.isSlot() && f.getSlot().getHadExplicitDefault() &&
  ...
  const c = f.createClassDeclaration([EXPORT], fullClassName, __, [
...
export function getImportNodes(
  ...
  return lookupNode(ctx, node)
    ...
    .filter((n) =>
      lookupNode(ctx, n).isStruct() || lookupNode(ctx, n).isEnum()
    );
}
