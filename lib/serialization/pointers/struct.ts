...
import { format, padToWord } from "../../utils.ts";
...
import { PointerType } from "./pointer_type.ts";
...
export class Struct<T = unknown> extends Pointer {
...
export function getFloat32(
  ...
  const v = ds.segment.getUint32(ds.byteOffset + byteOffset) ^
...
export function getFloat64(
  ...
  if (defaultMask !== undefined) {
    const lo = ds.segment.getUint32(ds.byteOffset + byteOffset) ^
      defaultMask.getUint32(0, true);
    const hi = ds.segment.getUint32(ds.byteOffset + byteOffset + 4) ^
...
export function getInt16(
  ...
  const v = ds.segment.getUint16(ds.byteOffset + byteOffset) ^
  ...
  if (isNull(l)) {
    ...
    if (
      ...
      for (let i = 0; i < srcLength; i++) {
        const srcElementOffset = srcContent.byteOffset +
          i * getByteLength(srcSize);
        const dstElementOffset = dstContent.byteOffset +
          i * getByteLength(dstSize);
        ...
        for (let j = 0; j < srcSize.pointerLength; j++) {
          ...
          const a = srcPtrTarget.segment.getUint8(srcPtrTarget.byteOffset) &
            0x03;
          const b = srcPtrTarget.segment.getUint32(srcPtrTarget.byteOffset + 4);
...
export function setFloat32(
  ...
  if (defaultMask !== undefined) {
    ...
    const v = TMP_WORD.getUint32(0, NATIVE_LITTLE_ENDIAN) ^
      defaultMask.getUint32(0, true);
...
export function setFloat64(
  ...
  if (defaultMask !== undefined) {
    ...
    const lo = TMP_WORD.getUint32(0, NATIVE_LITTLE_ENDIAN) ^
      defaultMask.getUint32(0, true);
    const hi = TMP_WORD.getUint32(4, NATIVE_LITTLE_ENDIAN) ^
      defaultMask.getUint32(4, true);
...
export function setInt16(
  ...
  if (defaultMask !== undefined) {
    ...
    const v = TMP_WORD.getUint16(0, NATIVE_LITTLE_ENDIAN) ^
      defaultMask.getUint16(0, true);
...
export function checkDataBounds(
  ...
  if (
    byteOffset < 0 || byteLength < 0 || byteOffset + byteLength > dataByteLength
  ) {
    throw new Error(
      format(
        PTR_STRUCT_DATA_OUT_OF_BOUNDS,
