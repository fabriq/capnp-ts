...
import { bufferToHex, format, padToWord } from "../../utils.ts";
...
import { PointerAllocationResult } from "./pointer_allocation_result.ts";
...
export function dump(p: Pointer): string {
  ...
  return `${pHex} > ${
    bufferToHex(f.segment.buffer.slice(f.byteOffset, f.byteOffset + 8))
  }`;
}
...
function isCompositeList(p: Pointer): boolean {
  return getTargetPointerType(p) === PointerType.LIST &&
...
export function isDoubleFar(p: Pointer): boolean {
  return getPointerType(p) === PointerType.FAR &&
  ...
  if (srcElementSize === ListElementSize.POINTER) {
    ...
    for (let i = 0; i < srcLength; i++) {
      for (let j = 0; j < srcCompositeSize.pointerLength; j++) {
        const offset = i * srcStructByteLength +
          srcCompositeSize.dataByteLength + (j << 3);
